...
// 准备附件数据
if ($post->have()) {
    \Widget\Contents\Attachment\Related::alloc(['parentId' => $post->cid])->to($attachment);
} else {
...
<div class="container-fluid">
    <div class="row rounded-pill">
        <div class="col-12">
            ...
            <!-- 主表单 -->
            <form action="<?php $security->index('/action/contents-post-edit'); ?>" method="post" name="write_post" id="write_post">
                <div class="row g-4">
                    ...
                    <div class="col-lg-9">
                        ...
                        <?php if ($post->draft): ?>
                            <div class="alert alert-warning alert-dismissible fade show border-0 shadow-sm" role="alert">
                                ...
                                <?php if ($post->draft['cid'] != $post->cid):
                                    $postModifyDate = new \Typecho\Date($post->draft['modified']);
                                    _e('当前正在编辑的是保存于 <strong>%s</strong> 的草稿，你可以 <a href="%s" class="alert-link edit-draft-notice">删除它</a>', $postModifyDate->word(), 
                        ...
                        <div class="card-modern">
                            <div class="card-body p-4">
                                ...
                                if ($post->have()) {
                                    $permalink = str_replace(['{cid}', '{category}', '{year}', '{month}', '{day}'],
                                    [$post->cid, $post->category, $post->year, $post->month, $post->day], $permalink);
                                }
                                $input = '<span class="slug-input-wrapper"><input type="text" id="slug" name="slug" autocomplete="off" value="' . htmlspecialchars($post->slug ?? '') . '" class="form-control form-control-sm border-0 bg-light d-inline-block w-auto mx-1" style="min-width:100px;" /></span>';
                                ?>
                        ...
                        <div class="card-modern mt-3">
                             <div class="card-body d-flex justify-content-between align-items-center submit">
                                ...
                                <div class="d-flex gap-2 right">
                                    ...
                                    <button type="submit" name="do" value="publish" id="btn-submit" class="btn btn-primary fw-bold rounded-pill px-4 shadow-sm"><i class="fa-solid fa-paper-plane me-1"></i> <?php _e('发布文章'); ?></button>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card-body p-0">
                                ...
                                <div class="tab-content px-3 pb-3">
                                    ...
                                    <div id="tab-advance" class="tab-pane fade show active">
                                        <div class="mb-3"><label for="date" class="form-label small text-muted fw-bold mb-1"><?php _e('发布日期'); ?></label><input class="form-control form-control-sm" type="text" name="date" id="date" autocomplete="off" value="<?php $post->have() && $post->created > 0 ? $post->date('Y-m-d H:i') : ''; ?>" /></div>
                                        <div class="mb-3"><label class="form-label small text-muted fw-bold mb-1"><?php _e('分类'); ?></label>
                                            <div class="card border bg-light p-2" style="max-height: 200px; overflow-y: auto;">
                                                <?php \Widget\Metas\Category\Rows::alloc()->to($category); ?>
                                                <ul class="list-unstyled mb-0">
                                                    <?php
                                                    $categories = ($post->have() ? array_column($post->categories, 'mid') : []);
                                                    while ($category->next()): ?>
                                                        <li class="form-check">
                                                            <input type="checkbox" class="form-check-input" id="category-<?php $category->mid(); ?>" value="<?php $category->mid(); ?>" name="category[]" <?php if (in_array($category->mid, $categories)): ?>checked="true"<?php endif; ?>/>
                                                            <label class="form-check-label small" for="category-<?php $category->mid(); ?>"><?php echo str_repeat('&nbsp;&nbsp;', $category->levels); ?><?php $category->name(); ?></label>
                                                        </li>
                                                    <?php endwhile; ?>
                                                </ul>
                                            </div>
                                        </div>
                                        <div class="mb-3"><label for="tags" class="form-label small text-muted fw-bold mb-1"><?php _e('标签'); ?></label><input id="tags" name="tags" type="text" value="<?php $post->tags(',', false); ?>" class="form-control form-control-sm" placeholder="<?php _e('逗号分隔'); ?>" /></div>
                                                ...
                                                <div id="collapseAdv" class="accordion-collapse collapse">
                                                    <div class="accordion-body p-2 pt-3">
                                                        <div id="advance-panel"> <!-- Wrapper needed by old JS -->
                                                            ...
                                                            <div id="post-password" class="mb-2 <?php if (strlen($post->password ?? '') == 0): ?>d-none<?php endif; ?>"><input type="text" name="password" id="protect-pwd" class="form-control form-control-sm" value="<?php $post->password(); ?>" placeholder="<?php _e('内容密码'); ?>" /></div>
                                                            <div class="mb-2">
                                                                <label class="form-label small text-muted mb-1"><?php _e('权限控制'); ?></label>
                                                                <div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="allowComment" name="allowComment" value="1" <?php if ($post->allow('comment')): ?>checked="true"<?php endif; ?>><label class="form-check-label small" for="allowComment"><?php _e('允许评论'); ?></label></div>
                                                                <div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="allowPing" name="allowPing" value="1" <?php if ($post->allow('ping')): ?>checked="true"<?php endif; ?>><label class="form-check-label small" for="allowPing"><?php _e('允许被引用'); ?></label></div>
                                                                <div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="allowFeed" name="allowFeed" value="1" <?php if ($post->allow('feed')): ?>checked="true"<?php endif; ?>><label class="form-check-label small" for="allowFeed"><?php _e('允许在聚合中出现'); ?></label></div>
                                                            </div>
                                                            <div class="mb-2">
                                                                <label for="trackback" class="form-label small text-muted mb-1"><?php _e('引用通告'); ?></label>
                                                                <textarea id="trackback" class="form-control form-control-sm" name="trackback" rows="2"></textarea>
                                                                <p class="form-text small mt-1"><?php _e('每一行一个引用地址, 用回车隔开'); ?></p>
                                                            </div>
                                                    ...
                                                    <div class="d-flex align-items-center w-100">
                                                        ...
                                                        <div class="flex-grow-1 min-width-0">
                                                            ...
                                                            <div class="small text-muted btn-group btn-group-sm opacity-50 hover-opacity-100">
                                                                <a class="btn text-secondary file" target="_blank" href="<?php $options->adminUrl('media.php?cid=' . $attachment->cid); ?>" title="<?php _e('编辑'); ?>"><i class="fa-solid fa-pen"></i></a>
                                                                <a class="btn text-danger delete" href="###" title="<?php _e('删除'); ?>"><i class="fa-solid fa-trash"></i></a>
                                                            </div>
                                                        </div>
    ...
    (function() {
        ...
        $('a[href="#tab-files"]').one('shown.bs.tab', function() {
            ...
            uploader = new plupload.Uploader({
                ...
                url: '<?php $security->index('/action/upload' . ($post->have() ? '?cid=' . $post->cid : '')); ?>',
...
/* FIXED: For Tag Input (token-input.js) */
ul.token-input-list-bootstrap { overflow: hidden; height: auto !important; width: 100%; border: 1px solid #ced4da; cursor: text; font-size: .875rem; min-height: calc(1.5em + .5rem + 2px); z-index: 999; margin: 0; padding: 2px 4px; background-color: #fff; list-style-type: none; clear: left; border-radius: .375rem; display: flex; flex-wrap: wrap; align-items: center; }
ul.token-input-list-bootstrap li { list-style-type: none; }
ul.token-input-list-bootstrap li input { border: 0; width: 100px; padding: .2rem; background-color: transparent; -webkit-appearance: caret; font-size: .875rem; }
ul.token-input-list-bootstrap li.token-input-token-bootstrap { overflow: hidden; height: auto; margin: 3px; padding: 3px 8px; background-color: #e9ecef; color: #495057; border: 1px solid #ced4da; font-size: 12px; border-radius: 5px; white-space: nowrap; }
li.token-input-token-bootstrap p { display: inline; padding: 0; margin: 0; }
li.token-input-token-bootstrap span { color: #6c757d; margin-left: 5px; font-weight: bold; cursor: pointer; }
.token-input-dropdown-bootstrap { position: absolute; background-color: #fff; overflow: hidden; border: 1px solid #ced4da; cursor: default; z-index: 1051; box-shadow: 0 .5rem 1rem rgba(0,0,0,.15); border-radius: .3rem; }
.token-input-dropdown-bootstrap ul { list-style: none; margin: 0; padding: .5rem 0; }
.token-input-dropdown-bootstrap ul li { padding: .25rem 1rem; }
.token-input-dropdown-bootstrap ul li.token-input-selected-dropdown-item-bootstrap { background-color: var(--primary-color); color: #fff; }
