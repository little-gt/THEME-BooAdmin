...
[![BooAdmin](https://img.shields.io/badge/BooAdmin-v1.1.8--PRE-blue?style=for-the-badge)](https://github.com/little-gt/THEME-BooAdmin/releases)
...
> **⚠️ PRE 版本特别说明：** 1.1.8-PRE 是一个预发布版本，包含了即将发布的正式版本的功能预览。建议在测试环境中使用，不推荐在生产环境中部署。

- 修复了发布文章和发布独立页面时，错误弹出的，是否离开页面而不保存的提示；
- 修复了下拉选项框焦点获取异常，导致难以或者无法选中下拉选项的问题；
- 修复了管理用户资料页面头像加载失败时，导致的页面元素异常问题，引入了一致性的头像退化；
- 统一了管理评论页面的头像加载失败的退化机制，修复了回复评论表单无法显示评论者头像问题。
...
> **安装建议：** 如果您是首次安装，建议使用正式版本；如果您希望体验最新功能并帮助测试，请使用 PRE 版本并在 [GitHub Issues](https://github.com/little-gt/THEME-BooAdmin/issues) 中反馈遇到的问题。
