...
[![BooAdmin](https://img.shields.io/badge/BooAdmin-v1.1.9--PRE-blue?style=for-the-badge)](https://github.com/little-gt/THEME-BooAdmin/releases)
...
### 🚀 v1.1.9 版本 更新速递
- ⏳ **加载指示器优化**: 调整加载指示器位置到页面右下角，并且提升其视觉效果；
- ⚙️ **主题编辑器优化**: 主题编辑器支持全屏编辑，以及保存时候的二次确认。
...
### 🎨 TailwindCSS 完全重构
BooAdmin v1.1.x 系列版本，采用业界领先的 **TailwindCSS** 框架进行完全重构，相比传统 CSS 框架提供：
...
### v1.1.8 版本
- 🐛 **页面离开提示修复**: 修复了发布文章和发布独立页面时，错误弹出的是否离开页面而不保存的提示问题；
- 🎯 **下拉选项框优化**: 修复了下拉选项框焦点获取异常，导致难以或者无法选中下拉选项的问题；
- 🖼️ **头像加载异常修复**: 修复了管理用户资料页面头像加载失败时导致的页面元素异常问题，引入了一致性的头像退化机制；
- 💬 **评论头像降级统一**: 统一了管理评论页面的头像加载失败的退化机制，修复了回复评论表单无法显示评论者头像问题；
- ⏳ **加载指示器优化**: 优化了加载指示器的视觉效果和性能；
- 📌 **侧边栏位置记忆**: 侧边栏现在会记住当前的位置，而不会每次都返回顶部。
...
- 🖱️ **撰写文章页面按钮设计优化**: 优化了撰写文章页面的按钮设计，使其风格更加统一，提升用户体验。

### v1.1.6 版本
- 🎯 **Header 导航统一**: 统一所有页面的 Header 导航条显示内容，提供更加统一和自然的浏览体验；
- 🎨 **主题编辑优化**: 优化编辑主题页面的设计，与设置主题页面样式保持一致；
- 🔧 **GitHub 跳转修复**: 修复 Copyright 弹窗点击跳转到 GitHub 仓库无响应的问题；
- 💬 **评论卡片视图**: 新增评论管理页面卡片视图支持，提供更直观的评论浏览方式；
- 💾 **视图逻辑统一**: 统一所有卡片视图的存储逻辑和切换逻辑，保持一致性体验；
- 🖼️ **头像降级优化**: 优化侧边栏和首页的头像降级功能，当头像无法加载时，自动显示 FontAwesome 图标作为降级方案。
